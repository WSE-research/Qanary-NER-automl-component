...
def calculate_values_for_two_lists_with_same_key (expected, resulted):
    ...
    if len(expected) > 0 or len(resulted) > 0:
        ...
        inc_current = inc_current + inc
        mis_current = mis_current + mis
        spu_current = spu_current + spu
...
def save_current_results(title):
    ...
    generate_result_text(title=title, cor=cor_current, inc=inc_current, mis=mis_current, spu=spu_current)

    cor_total = cor_total + cor_current
    inc_total = inc_total + inc_current
    mis_total = mis_total + mis_current
    spu_total = spu_total + spu_current
    cor_current = 0
    inc_current = 0
    mis_current = 0
    spu_current = 0
